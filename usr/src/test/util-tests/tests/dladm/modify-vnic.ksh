...
# This test exercises the 'dladm modify-vnic' command and the underlying
# operations of libdladm and the vnic driver.
#
# The test creates a vnic and then modifies it for various differing
# configurations, some of which are illegal and expected to fail. For those
# cases that are expected to succeed, the test verifies that the persistent
# and active configurations match what was set. Afterwards, the test deletes
# the created vnic device and verifies it is no longer present in the active
...
dl_configs["fail_dup_v"]="-v ${dl_vlan[vlan_1]} -v ${dl_vlan[vlan_2]}"
...
dl_extra_configs["vlan_1"]="-v ${dl_vlan[vlan_1]}"
dl_extra_configs["vlan_2"]="-v ${dl_vlan[vlan_2]}"
dl_extra_configs["vlan_3"]="-v ${dl_vlan[vlan_3]}"
...
test_modify()
{
    ...
    typeset config="${dl_configs[${conf}]} ${dl_extra_configs[${extra}]}"
    typeset modify="${dl_modify}"
    typeset out
    typeset ret

    [[ ! -z "${args}" ]] && modify="${dl_modify} ${args}"

    out=$(${modify} ${config} ${dl_vnic} 2>&1)
    ret=$?

    if [[ "${conf:0:4}" == "fail" ]]; then
        if (( ret == 0 )); then
            warn "'${modify}' [${conf}, ${extra}] returned ${ret}"
        fi
        return ${ret}
    else
        if (( ret != 0 )); then
...
compare()
{
    ...
    if [[ "${val}" != "${exp}" ]]; then
        warn "'${cmd}' [${conf}, ${extra}] returned wrong value for ${name}: " \
             "'${val}' (expected '${exp}')"
        return 1
    fi
    return 0
...
test_delete()
{
    ...
    if (( ret != 0 )); then
	warn "'${dl_delete}' returned ${ret}, error message:\n${out}"
        return ${ret}
    fi
    ...
    if (( ret == 0 )); then
	warn "'${dl_show}' after '${dl_delete}' returned ${ret}, " \
             "output:\n${out}"
        return ${ret}
    fi
    ...
    if (( ret == 0 )); then
	warn "'${dl_show} -P' after '${dl_delete}' returned ${ret}, " \
             "output:\n${out}"
        return ${ret}
    fi
}
